...
-- https://github.com/streetturtle/awesome-wm-widgets/tree/master/micro-widget
...
local utils = require("ui.wibar.module.aww.micro-widget.utils")

local LIST_DEVICES_CMD = [[sh -c "pacmd list-sources"]]
...
local widget_types = {
	icon_and_text = require("ui.wibar.module.aww.micro-widget.widgets.icon-and-text-widget"),
	icon = require("ui.wibar.module.aww.micro-widget.widgets.icon-widget"),
	arc = require("ui.wibar.module.aww.micro-widget.widgets.arc-widget"),
	horizontal_bar = require("ui.wibar.module.aww.micro-widget.widgets.horizontal-bar-widget"),
	vertical_bar = require("ui.wibar.module.aww.micro-widget.widgets.vertical-bar-widget"),
}
local micro = {}
...
local function rebuild_popup()
	spawn.easy_async(LIST_DEVICES_CMD, function(stdout)
		local sources = utils.extract_sinks_and_sources(stdout)
...
local function worker(user_args)
	...
	local mixctrl = args.mixctrl or "Capture"
	local value_type = args.value_type or "-M"
	local toggle_cmd = args.toggle_cmd or nil

	if widget_types[widget_type] == nil then
		micro.widget = widget_types["icon_and_text"].get_widget(args.icon_and_text_args)
	else
		micro.widget = widget_types[widget_type].get_widget(args)
	end
	...
	local function update_graphic(widget, stdout)
		...
		local micro_level = string.match(stdout, "(%d?%d?%d)%%") -- (\d?\d?\d)\%)
		micro_level = string.format("% 3d", micro_level)
		widget:set_micro_level(micro_level)
	end

	function micro:inc(s)
		spawn.easy_async(INC_VOLUME_CMD(card, device, mixctrl, value_type, s or step), function(stdout)
			update_graphic(micro.widget, stdout)
		end)
	end

	function micro:dec(s)
		spawn.easy_async(DEC_VOLUME_CMD(card, device, mixctrl, value_type, s or step), function(stdout)
			update_graphic(micro.widget, stdout)
		end)
	end

	function micro:toggle()
	...
	function micro:mixer()
		if mixer_cmd then
			spawn.easy_async(mixer_cmd)
		end
	end

	micro.widget:buttons(awful.util.table.join(
	...
	watch(GET_VOLUME_CMD(card, device, mixctrl, value_type), refresh_rate, update_graphic, micro.widget)

	return micro.widget
end

return setmetatable(micro, {
	__call = function(_, ...)
		return worker(...)
	end,
})
